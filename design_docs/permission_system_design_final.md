...
| `menu_type` | CHAR(1) | | 'D'=Directory, 'C'=Component, 'F'=Function |
...
**Request Example**:
```http
GET /api/v1/auth/info?username=admin
```
...
**Request Example**:
```http
GET /api/v1/menu/user/e277662d-428f-4030-84d4-f167c8a4610f
```
...
**Purpose**: Get all menus (for role management)
...
**Purpose**: Get all users who act as line managers. (determined by users assigned the role with name `LM`)
**Response**:
```json
[
  {
...
```json
{
  "staffId": "40001",
  "name": "David Staff",
  "roles": ["29adfeab-0e5d-48ab-85db-64800622053a"],
  "status": 1,
...
```json
{
  ...
  "staffId": "40001",
  "name": "Alice Cooper",
  "roles": ["29adfeab-0e5d-48ab-85db-64800622053a", "29adfeab-0e5d-48ab-85db-64800622053e"],
  "lineManagerId": "29adfeab-0e5d-48ab-85db-648006220531",
...
**Usage**:
- **Sidebar**: Only shows "System Management > User Management".
...
5. **Menu Types**:
   - Use 'D' for categories with children
...
## 10. UI Design Wireframes

### 10.1 Role Management Interface

**Goal**: Visualize the Role Management layout where admins can create roles and assign permissions via a tree structure.

```mermaid
graph TD
    subgraph "Role Management Page Layout"
        direction TB
        Header[Header: Role Management]
        
        subgraph "Main Content"
            direction LR
            
            subgraph "Left Panel: Role List"
                RoleList[Role List Table]
                BtnAdd["Button: + Create Role"]
                R1["Role: Admin (Active)"]
                R2["Role: QC Manager (Active)"]
                R3["Role: Staff (Active)"]
            ...
            subgraph "Right Panel: Role Details & Permissions"
                Form[Form: Role Details]
                FName[Input: Role Name]
                FDesc[Input: Description]
                FStatus[Switch: Active/Inactive]
                
                TreeTitle["Label: Menu Permissions"]
                Tree["Tree Component: Selectable Checkboxes"]
                
                T1["x Dashboard"]
                T2["x QC Module"]
                T21["x Sampling"]
                T22["x Inbox"]
                T22_F1["x Button: Delete"]
                T22_F2["  Button: Export"]
    ...
    style BtnAdd fill:#4CAF50,stroke:#333,stroke-width:2px;
    style BtnSave fill:#2196F3,stroke:#333,stroke-width:2px;
    style T1 fill:#e1f5fe
    style T21 fill:#e1f5fe
    style T22_F1 fill:#e1f5fe
```

### 10.2 User Management - Role Assignment

**Goal**: Visualize the modal dialog for assigning roles to a user.

```mermaid
graph TD
    subgraph "Edit User Modal"
        direction TB
        Title[Title: Edit User: Alice Staff]
        
        Content[Form Content]
        InputName["Input: Staff Name (Read-only)"]
        InputID["Input: Staff ID (Read-only)"]
        
        DropdownRole["Select: Assigned Role"]
        ListRoles["Dropdown Options: Admin, QC Manager, Staff"]
        
        DropdownMgr["Select: Line Manager"]
        ListMgrs["Dropdown Options: Super Manager, Bob Manager"]
        
        Actions[Footer Actions]
        BtnOk[Button: Confirm]
        BtnClose[Button: Cancel]
        
        Title --> Content
        Content --> InputName
        Content --> InputID
        Content --> DropdownRole
        DropdownRole -.-> ListRoles
        Content --> DropdownMgr
        DropdownMgr -.-> ListMgrs
    ...
    style BtnOk fill:#2196F3,color:white
```

### 10.3 Menu Management

**Goal**: Visualize the detailed tree-table for managing menu structure.

```mermaid
graph TD
    subgraph "Menu Management Table View"
        direction TB
        Toolbar["Toolbar: + Add Menu | Refresh | Expand All"]
        
        TableHeader["Cols: Menu Name | Type | Icon | Permission | Visible | Sort | Actions"]
        
        Row1["Row 1: > QC Module"]
        Cell1_Type[Dir]
        Cell1_Icon[Folder]
        Cell1_Act["Edit | Add Sub"]
        
        Row2["Row 2:   v QC Inbox (Child of QC Module)"]
        Cell2_Type[Menu]
        Cell2_Icon[Mail]
        Cell2_Perm[qc:inbox:list]
        Cell2_Act["Edit | Add Sub"]
        
        Row3["Row 3:     Delete Button (Child of QC Inbox)"]
        Cell3_Type[Btn]
        Cell3_Icon[-]
        Cell3_Perm[qc:inbox:delete]
        Cell3_Vis[Hidden]
        Cell3_Act["Edit | Delete"]
...
### 11.1 User Login & Dynamic Menu Loading

This sequence describes how the frontend retrieves and renders the personalized menu after login.

```mermaid
sequenceDiagram
    participant U as User
    participant app as Frontend (React)
    participant api_auth as Auth Controller
    participant api_menu as Menu Controller
    participant db as Database
    
    U->>app: 1. Enters Credentials
    app->>api_auth: 2. POST /api/auth/login
    api_auth->>db: Validate User
    db-->>api_auth: User OK
    api_auth-->>app: Return JWT Token
    
    app->>api_auth: 3. GET /api/auth/info (w/ Token)
    api_auth->>db: Fetch User & Role Status
    db-->>api_auth: User + Role Details
    api_auth-->>app: Return User Info (excl. menus)
    
    app->>api_menu: 4. GET /api/menu/user/{uid}
    api_menu->>db: Query Menus for User's Roles
    db-->>api_menu: List of Menus
    api_menu->>api_menu: Build Tree Structure (O(n))
    api_menu-->>app: Return Menu Tree JSON
    
    app->>app: 5. Generate Routes & Sidebar
    app-->>U: 6. Display Dashboard
```

### 11.2 Role Creation & Permission Assignment

This sequence describes the admin process of creating a new role and assigning permissions.

```mermaid
sequenceDiagram
    participant Admin
    participant page as Role Page
    participant api_role as Role Controller
    participant api_menu as Menu Controller
    participant db as Database
    
    Admin->>page: 1. Click "Create Role"
    page->>api_menu: 2. GET /api/menu/list (All Menus)
    api_menu->>db: Fetch All Menus
    db-->>api_menu: Return Result
    api_menu-->>page: Return Flat Menu List
    
    page->>page: 3. Render Permission Tree
    Admin->>page: 4. Enter Role Name & Check Permissions
    Admin->>page: 5. Click "Save"
    
    page->>api_role: 6. POST /api/role
    Note right of page: Payload: { roleName: "QC Lead",<br/>menuIds: [1, 10, 11] }
    
    api_role->>db: Insert SysRole
    api_role->>db: Insert SysRoleMenu (Batch)
    db-->>api_role: Success
    api_role-->>page: 200 OK
    
    page-->>Admin: Show Success Message
    page->>page: Refresh Role List
```

### 11.3 Hybrid Permission Check (Field Level)

This sequence illustrates the "Two-Lock" mechanism: Frontend UX guidance and Backend security enforcement.

```mermaid
sequenceDiagram
    participant User
    participant ui as QC Detail Page
    participant api as QC Controller
    participant db as Database
    
    User->>ui: 1. Open QC Record #123
    ui->>ui: Check User Permissions (from Store)
    
    alt Has "qc:record:edit"
        ui->>ui: Render "Edit" Button
    else No Permission
        ui->>ui: Hide "Edit" Button
    end
    
    User->>ui: 2. Click "Edit" and Modify Score
    User->>ui: 3. Click "Save"
    
    ui->>api: 4. PUT /api/qc/record/123
    Note right of ui: Payload: { score: 98 }
    
    api->>api: Security Check @PreAuthorize("qc:record:edit")
    
    alt Permission Valid
        api->>db: Update Record
        db-->>api: Success
        api-->>ui: 200 OK
        ui-->>User: "Saved Successfully"
